...
static unsigned char dp_download_percent_control_handle(const unsigned char value[], unsigned short length)
{
    ...
    if (ret == SUCCESS)
    {
        global_time = (float)variable.global_percent / 100 * (float)variable.full_running_time;
        local_time = (float)current_percent / 100 * (float)variable.full_running_time;
       
        if (variable.global_percent < current_percent) //
        {
            runtime = local_time - global_time;
            open_curtain();
            SET_LED(RESET);
        }
        if (variable.global_percent > current_percent)
        {
            runtime = global_time - local_time;
            close_curtain();
            SET_LED(RESET);
        }

        ms_delay_systick(runtime);
        motor_stop();
        SET_LED(SET);
...
//电流阈值设定
static unsigned char dp_download_curtain_value_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为STRING
    unsigned char ret;
    unsigned long string_data = 0;
...
static unsigned char dp_download_curtain_operation_handle(const unsigned char value[], unsigned short length)
{
    ...
    if (ret == SUCCESS)
    {
        ...
        calibration_operation_auto = 0; //设置为手动校准
        return SUCCESS;
    }
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid, const unsigned char value[], unsigned short length)
{
  ...
  ***********************************/
    ...
    case DPID_CURTAIN_MODE: /*窗帘模式*/
        ...
        break;
    case DPID_CALIBRATION_STATUS: /*窗帘校准状态*/
