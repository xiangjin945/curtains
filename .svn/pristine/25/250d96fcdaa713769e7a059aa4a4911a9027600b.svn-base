...
void send_command(void)
{
	...
	{
		update_status(tmp_buf,len);
		calibration_state_68_pack(tmp_buf);
	}
	...
	{
		update_status(tmp_buf,len);
		calibration_operation_6D_pack(tmp_buf);
	}
...
void full_running_time_0A_pack(u32 run_time_ms)
{
	u8 crc8 = 0;
    ...
    dp_data_buf[8] = 0x00;
    dp_data_buf[9] = 0x04;
    dp_data_buf[10] = variable.full_running_time>>24;
    dp_data_buf[11] = variable.full_running_time>>16;
    dp_data_buf[12] = variable.full_running_time>>8;
    dp_data_buf[13] = variable.full_running_time;
...
void rf_test(void)
{
	...
	tmp_buf = strstr((char*)uart0_buf,"true");
	if(tmp_buf==NULL)
	{
		//没有搜索到该信号
	}
...
void tybn1_join_sleep_mode(void)
{
	...
	ms_delay_systick(5000);			// NOTE: 延迟n秒
}
